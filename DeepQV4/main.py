...
class TradingEnvironment:
    ...
    def step(self, action_idx):
        ...
        # small reward based on unrealised pnl to encourage right direction
        if self.holdings > 0:
...
def validate_in_segments(
    ...
    for seg_idx, start in enumerate(segment_starts, 1):
        ...
        segment_data = full_val_data.iloc[start:end].copy()
        ...
        (
            ...
            hold_hist,
            trades,
        ) = run_episode(val_env, agent, segment_data, batch_size, is_eval=True)
        ...
        final_pv = pv_hist[-1]
        seg_return = (final_pv - initial_credit) / initial_credit * 100
...
def main():
    ...
    VAL_EVAL_WINDOW_MINUTES = 48 * 60  # 1 day
    ...
    for e in range(EPISODES):
        ...
        run_episode(train_env, agent, episode_data, BATCH_SIZE, is_eval=False)
        agent.decay_epsilon()
        ...
        plot_results(
            ...
            segment_boundaries=seg_starts[1:],  # do not draw line at index 0
        )
    ...
    pv, cred, hold, trades = run_episode(
        test_env, agent, test_data, BATCH_SIZE, is_eval=True
    )
    plot_results(
        test_data,
        episode=EPISODES,
