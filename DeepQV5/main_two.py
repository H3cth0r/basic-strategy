...
class TradingEnvironment:
    ...
    def step(self, action_idx):
        ...
        prev_portfolio_value = self.credit + self.holdings * current_price
        ...
        if action < 0:
            ...
            if sell_amount_in_usd > self.min_trade_amount_sell:
                ...
                # Calculate profit margin before selling
                profit_per_unit = current_price - self.average_buy_price
                profit_margin = profit_per_unit / self.average_buy_price if self.average_buy_price > 0 else 0
                
                self.credit += sell_amount * current_price * (1 - self.fee)
                self.holdings -= sell_amount
                
                # Reward based on profit margin and amount
                realized_pnl = profit_per_unit * sell_amount
                
                # Strong reward for profitable sells
                if profit_margin > 0:
                    reward += realized_pnl * (1 + profit_margin * 10)  # Amplify by profit margin
                else:
                    reward += realized_pnl * 0.5  # Smaller penalty for losses
                
                # Additional reward for growing credit
                credit_growth = (self.credit - self.max_credit) / self.max_credit if self.max_credit > 0 else 0
                if credit_growth > 0:
                    reward += credit_growth * 20.0  # Strong reward for credit growth
                    self.max_credit = self.credit
        ...
        # ----- BUY
            ...
            if investment > self.min_trade_amount_buy:
                # Only buy if we have decent credit or if holdings are low
                holdings_ratio = (self.holdings * current_price) / prev_portfolio_value if prev_portfolio_value > 0 else 0
                
                # Penalize buying when already heavily invested
                if holdings_ratio > 0.7:
                    reward -= 0.1
        ...
        # ----- HOLD
            ...
            # Penalize holding when there are significant unrealized profits
            if self.holdings > 0 and self.average_buy_price > 0:
                unrealized_profit_margin = (current_price - self.average_buy_price) / self.average_buy_price
                
                # Penalize holding with high unrealized profits
                if unrealized_profit_margin > 0.02:  # More than 2% profit
                    reward -= 0.05 * unrealized_profit_margin
                # Small penalty for general inaction
                reward -= 0.01
        ...
        # Portfolio value change reward (smaller weight)
        portfolio_change = (portfolio_value - prev_portfolio_value) / prev_portfolio_value if prev_portfolio_value != 0 else 0
        reward += portfolio_change * 2.0
    
        # Track unrealized losses as negative reward
        if self.holdings > 0 and self.average_buy_price > 0:
            current_unrealized = (next_price - self.average_buy_price) * self.holdings
            if current_unrealized < 0:
                reward += current_unrealized * 0.01  # Small penalty for unrealized losses
        ...
        if done:
            ...
            # Final reward based on credit growth
            final_credit_growth = (self.credit - self.initial_credit) / self.initial_credit if self.initial_credit != 0 else 0
            reward += final_credit_growth * 50.0
...
def main():
    ...
    ATTENTION_DIM = 32
    ATTENTION_HEADS = 4
    LEARNING_RATE = 0.0005  # Changed from 0.0001
    TARGET_UPDATE = 5
    WINDOW_SIZE = 180
    MIN_TRADE_AMOUNT_BUY = 1
    MIN_TRADE_AMOUNT_SELL = 1
    INITIAL_CREDIT = 100
    EPSILON_DECAY = 0.997  # Changed from 0.995
    ...
    if pv:
        final_pv = pv[-1]
        test_return = (final_pv - INITIAL_CREDIT) / INITIAL_CREDIT * 100 if INITIAL_CREDIT != 0 else 0
        buys = len([t for t in trades if t["type"] == "buy"])
        sells = len([t for t in trades if t["type"] == "sell"])
        print(
